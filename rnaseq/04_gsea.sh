...
do
	...
	# Cell types
	${GSEA} GSEAPreranked -gmx ${INPUT_PATH}/genome/msigdb_mouse/m8.all.v2023.2.Mm.symbols.gmt -rnk ${INPUT_PATH}/rnaseq/gsea/genes_${TIME}.ranked.rnk \
		-set_max 1000000 -set_min 0 -rpt_label Cell_type_${TIME} -out ${INPUT_PATH}/rnaseq/gsea/ \
		-collapse No_Collapse -mode Max_probe -norm meandiv -nperm 10000 -scoring_scheme weighted \
	...
	${GSEA} GSEAPreranked -gmx ${TARGETS_PATH}/genes/ZFP143_target_genes.gmx \
		-rnk ${INPUT_PATH}/rnaseq/gsea/genes_${TIME}.ranked.ensid.rnk \
		-set_max 1000000 -set_min 0 -rpt_label ZFP143_targets_${TIME} -out ${INPUT_PATH}/rnaseq/gsea/ \
		-collapse No_Collapse -mode Max_probe -norm meandiv -nperm 10000 -scoring_scheme weighted \
		-create_svgs true -include_only_symbols true -make_sets true -rnd_seed timestamp -zip_report false
done
